#!/usr/bin/env node
const { solveCaptcha } = require('./browserless_aliyun_captcha_solver');

const FILES = ['/tmp/feilin.js', '/tmp/aliyun-pe.js', '/tmp/AliyunCaptcha.js'];

function pickSample(report, kind) {
  const rows = Array.isArray(report?.feilinRsLogs) ? report.feilinRsLogs : [];
  if (kind === 'success') {
    return rows.find((row) =>
      typeof row?.outputString === 'string' &&
      row.outputString !== 'null' &&
      typeof row?.arg0 === 'string' &&
      typeof row?.arg1 === 'string') || null;
  }
  return rows.find((row) =>
    row?.outputString === 'null' &&
    typeof row?.arg0 === 'string' &&
    typeof row?.arg1 === 'string') || null;
}

function summarizeRow(row) {
  const raTail = Array.isArray(row?.raTraceLogs) ? row.raTraceLogs.slice(-4) : [];
  return {
    label: row?.label || null,
    outputString: row?.outputString || null,
    outputLength: typeof row?.outputString === 'string' ? row.outputString.length : null,
    innerStages: row?.innerStages || null,
    cryptoTraceCount: Array.isArray(row?.cryptoTraceLogs) ? row.cryptoTraceLogs.length : 0,
    raTail: raTail.map((entry) => ({
      stage: entry?.stage || null,
      i: entry?.i ?? null,
      fType: entry?.fType || null,
      fPreview: entry?.fPreview ?? null,
      returnType: entry?.returnType || null,
      returnPreview: entry?.returnPreview || null,
    })),
  };
}

async function main() {
  const base = await solveCaptcha({
    files: FILES,
    loaderPath: FILES[2],
  });
  const success = pickSample(base, 'success');
  const nullRow = pickSample(base, 'null');
  if (!success) throw new Error('missing success sample');
  if (!nullRow) throw new Error('missing null sample');

  const replay = await solveCaptcha({
    files: FILES,
    loaderPath: FILES[2],
    raTraceTargets: [success.arg0, nullRow.arg0, nullRow.arg1],
    cryptoTraceTargets: [success.arg0, nullRow.arg0, nullRow.arg1],
    rsExperimentInputs: [
      { label: 'control-success', arg0: success.arg0, arg1: success.arg1, thisKind: 'undefined' },
      { label: 'control-null', arg0: nullRow.arg0, arg1: nullRow.arg1, thisKind: 'undefined' },
      { label: 'replace-arg0-with-key', arg0: success.arg0, arg1: nullRow.arg1, thisKind: 'undefined' },
      { label: 'break-success-arg0', arg0: nullRow.arg0, arg1: success.arg1, thisKind: 'undefined' },
    ],
  });

  console.log(JSON.stringify({
    hypothesis: 'ra(opcode=20) gates on arg0 length == 16; arg0 length 32 flips f=true and returns null before crypto path',
    seeds: {
      success: {
        arg0: success.arg0,
        arg0Length: success.arg0.length,
        arg1Length: success.arg1.length,
      },
      null: {
        arg0: nullRow.arg0,
        arg0Length: nullRow.arg0.length,
        arg1Length: nullRow.arg1.length,
      },
    },
    rows: Array.isArray(replay.rsExperiment) ? replay.rsExperiment.map(summarizeRow) : replay.rsExperiment,
  }, null, 2));
}

if (require.main === module) {
  main().catch((err) => {
    console.error(String(err && err.stack || err));
    process.exit(1);
  });
}
